...
export function TransactionForm({
  ...
  const watchedDestinationAccountId = form.watch("destination_account_id");
  const selectedAccount = accounts?.find(
    (account: Account) => account.id === watchedAccountId,
  );
  const destinationAccount = accounts?.find(
    (account: Account) => account.id === watchedDestinationAccountId,
  );
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    >
                      ...
                      <SelectContent>
                        {availableDestinationAccounts?.map((account: Account) => (
                          <SelectItem key={account.id} value={account.id}>
                            <div className="flex items-center justify-between w-full">
                              <span>{account.identification}</span>
                              <Badge variant="outline" className="ml-2 text-xs">
                    ...
                    <FieldDescription>
                      O valor será transferido da conta de origem para esta conta
                    </FieldDescription>
                    {fieldState.invalid && (
                      <FieldError errors={[fieldState.error]} />
                    )}
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            {isSubmitting && (
              <div className="mt-4 p-3 bg-muted rounded-md" role="status" aria-live="polite">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <div className="animate-pulse h-2 w-2 bg-primary rounded-full" aria-hidden="true" />
